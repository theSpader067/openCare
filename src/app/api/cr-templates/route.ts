  ...
  try {
    ...
    if (session?.user?.email) {
      ...
      if (user) {
        ...
        templates = await prisma.cR_template.findMany({
          where: {
            OR: [
              { isPublic: true },
              { creatorId: user.id },
    ...
    console.error("Error fetching CR templates:", error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { title, specialite, details, recommendationsPostop, isPublic } = body;

    if (!title || !specialite || !details) {
      return NextResponse.json(
        { error: "Missing required fields: title, specialite, details" },
        { status: 400 }
      );
    }

    const template = await prisma.cR_template.create({
      data: {
        title,
        specialite,
        details,
        recommendationsPostop: recommendationsPostop || null,
    ...
    console.error("Error creating CR template:", error);
    return NextResponse.json(
