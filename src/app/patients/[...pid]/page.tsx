...
const Sidebar: React.FC<{ isExpanded: boolean; setIsExpanded: (isExpanded: boolean) => void; currentPage: string; onNavigate: (page: string) => void }> = ({ isExpanded, setIsExpanded, currentPage, onNavigate }) => {
    ...
    return (
        <aside className={`hidden md:flex flex-col bg-white p-4 transition-all duration-300 ease-in-out ${isExpanded ? 'w-64' : 'w-20 items-center'}`}>
            ...
            <nav className="flex flex-col gap-4 w-full">{navItems.map((item) => (<a href="#" key={item.id} onClick={(e) => { e.preventDefault(); onNavigate(item.id); }} className={`sidebar-icon p-2 flex items-center gap-3 ${(item.id === 'patients' && isPatientRelatedPage) ? 'active' : ''} ${!isExpanded ? 'justify-center' : ''}`} title={item.title}><item.icon className="flex-shrink-0" />{isExpanded && <span className="truncate">{item.title}</span>}</a>))}</nav>
...
const MiniWysiwygEditor: React.FC<{ value: string; onChange: (value: string) => void }> = ({ value, onChange }) => {
    ...
    return (
        <div className="border border-gray-300 rounded-lg">
            <div className="flex items-center gap-1 border-b border-gray-300 rounded-t-lg p-1 bg-gray-50">
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-lg p-6 w-full max-w-sm shadow-xl">
            <div className="flex items-start gap-4">
                ...
                <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left"><h3 className="text-lg leading-6 font-medium text-gray-900">Supprimer l'épisode</h3><div className="mt-2"><p className="text-sm text-gray-500">Êtes-vous sûr de vouloir supprimer cet épisode ? Cette action est irréversible.</p></div></div>
            </div>
        ...
        <main className="flex-1 grid grid-cols-1 lg:grid-cols-4 gap-6 overflow-hidden">
            <aside className="lg:col-span-1 flex flex-col gap-6">
                <div className="bg-white p-6 rounded-xl shadow-sm text-center relative">
                    <button className="absolute top-4 left-4 text-gray-400 cursor-default"><ArrowLeft /></button>
...
const MobileNav: React.FC<{ currentPage: string, onNavigate: (page: string) => void }> = ({ currentPage, onNavigate }) => {
    const navItems = [ { id: 'patients', icon: Users, title: "Patients" }, { id: 'calendar', icon: CalendarIcon, title: "Calendrier" }, { id: 'messages', icon: MessageSquare, title: "Messages" }, { id: 'settings', icon: Settings, title: "Paramètres" }, ];
     const isPatientRelatedPage = currentPage === 'patients' || currentPage === 'patientFile';
    return (<nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 flex justify-around p-2 z-10">{navItems.map((item) => (<a href="#" key={item.id} onClick={(e) => { e.preventDefault(); onNavigate(item.id)}} className={`sidebar-icon p-3 ${(item.id === 'patients' && isPatientRelatedPage) ? 'active' : ''}`} title={item.title}><item.icon /></a>))}</nav>);
};

export default function MedicalDashboardApp() {
    const [isSidebarExpanded, setIsExpanded] = useState(true);
    const [currentPage, setCurrentPage] = useState('patientFile');
    const [activePatient, setActivePatient] = useState<Patient | null>(mockPatients[0]);
