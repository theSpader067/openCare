...
interface CRTemplatesSidebarProps {
  onSelectTemplate: (template: CRTemplate) => void;
  onOpenCreateTemplate?: () => void;
  refreshTrigger?: number;
}

interface BackendCRTemplate {
  id: number;
  title: string;
  specialite: string;
  details: string;
  recommendationsPostop: string | null;
  isPublic: boolean;
  creatorId: number;
}
...
export function CRTemplatesSidebar({
  onSelectTemplate,
  onOpenCreateTemplate,
  refreshTrigger,
}: CRTemplatesSidebarProps) {
  const [isOpen, setIsOpen] = useState(false);
  const [selectedTemplateId, setSelectedTemplateId] = useState<string | null>(null);
  const [allTemplates, setAllTemplates] = useState<BackendCRTemplate[]>([]);
  ...
  // Organize templates by specialty
  const categorizedTemplates = useMemo(() => {
    const categories: TemplateCategory[] = [];
    const categoryMap = new Map<string, CRTemplate[]>();

    // Group templates by specialty
    allTemplates.forEach((template) => {
      const specialty = template.specialite || "autres";
      if (!categoryMap.has(specialty)) {
        categoryMap.set(specialty, []);
      }
      categoryMap.get(specialty)!.push({
    ...
    // Convert map to array of categories, ordered by known specialties
    Object.entries(SPECIALTY_NAMES).forEach(([specialtyId, name]) => {
      if (categoryMap.has(specialtyId)) {
        categories.push({
          id: specialtyId,
          name,
          templates: categoryMap.get(specialtyId) || [],
        });
      }
    });

    // Add any custom specialties not in the predefined list
    categoryMap.forEach((templates, specialtyId) => {
      if (!SPECIALTY_NAMES[specialtyId]) {
        categories.push({
          id: specialtyId,
