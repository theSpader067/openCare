              ...
              {profileOpen ? (
                <div className="absolute right-0 top-full z-40 mt-2 w-72 rounded border border-slate-200 bg-white p-4 shadow-lg">
                  ...
                  <div className="flex items-center gap-3 border-b border-slate-200 pb-4 mb-4">
                    <div className="flex h-12 w-12 items-center justify-center rounded-lg bg-indigo-600 text-lg font-semibold text-white flex-shrink-0">
                    {(session.user as any).firstName && (session.user as any).lastName ? `${(session.user as any).firstName[0]}${(session.user as any).lastName[0]}` : (session.user as any).username ? (session.user as any).username[0] : "" }
                    </div>
                    <div>
                  ...
                  <div className="mb-4 flex items-center gap-3">
                    <label className="text-xs font-semibold uppercase tracking-wide text-slate-600 whitespace-nowrap">
                      {t('dashboard.header.language') || 'Language'}
                    </label>
                    <select
                      value={language}
                      onChange={(e) => setLanguage(e.target.value as 'en' | 'fr')}
                      className="flex-1 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-700 transition hover:border-slate-300 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                    >
                      <option value="en">English</option>
                      <option value="fr">Français</option>
                    </select>
                  </div>
                  <div className="border-b border-slate-200 pb-4 mb-4"></div>
                  <div className="mt-4 space-y-2">
                    <button
                      type="button"
                      className="flex w-full items-center gap-3 rounded-lg bg-slate-100 px-4 py-2.5 text-sm font-medium text-slate-700 transition hover:bg-slate-200 hover:text-slate-900"
                      onClick={() => {
                        setProfileOpen(false);
                        router.push("/profile");
                      }}
                    ...
                    >
                      <span className="h-4 w-4 text-slate-600 flex items-center justify-center">⚙</span>
                      Admin Panel
                    </button>
                    <button
                      type="button"
                      className="flex w-full items-center gap-3 rounded-lg bg-red-50 px-4 py-2.5 text-sm font-medium text-red-700 transition hover:bg-red-100 hover:text-red-900"
                      onClick={() => {
                        setProfileOpen(false);
